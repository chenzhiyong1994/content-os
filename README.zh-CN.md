...
Content OS 面向中文内容创作者，把选题发现、资料研究、微信公众号长文、人工校准和可选正文配图组织成一套有明确阶段、产物和确认点的工作流。
...
- **阶段产物可追溯**：从 `00_选题卡.md` 到 `05_最终稿.md` 和公众号发布准备，每一步都有稳定输入和输出。
- **研究与表达分离**：聚合摘要只做信号，不冒充事实来源；关键结论回到原始材料核验。
- **作者声音可积累**：用你自己的代表作、风格画像和人工反馈校准写作，而不是永久依赖一条“像某某那样写”。
- **人工修改只用于学习声音**：对比上传文本与修改后的措辞、标点、断句和分段，格式细节不进入风格画像。
...
flowchart TD
  A["选题发现（可选）"] --> B["研究素材"]
  B --> C["初稿 → 编辑修改 → 风格深化 → 最终稿"]
  C --> D["公众号草稿箱与人工审看"]
  D --> E["正文配图（可选）"]
  D -. "手动修改完成后" .-> F["纯文本回读，对照上传基线"]
  F --> G["作者风格画像"]
```

小红书改编与生图已停用，原指令保存在 `docs/archive/`，不再参与主流程路由。小红书作为选题信息来源继续保留。回读是独立的风格反馈支线，不影响发布或正文配图。

流程不是一定要全部跑完。热点收集和公众号正文配图是明确的可选节点；内容阶段则通过 Review Gate 逐步确认，避免一次性跑到底。

## 适合谁

- 已经在写公众号或知识型内容，希望 AI 不再把声音磨平的人。
...
需要 Node.js 22.18+ 与 PowerShell 7+。`npm ci` 安装锁定的 HTML 文本抽取依赖，测试使用 Node.js 内置测试运行器。预览和试运行结果按文章隔离。
...
| ImageGen | 封面和公众号正文图 | 展示工具路径、可见模型信息、尺寸、输出路径和完整 prompt 后再确认 |
